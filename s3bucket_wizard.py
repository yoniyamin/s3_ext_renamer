...
def signal_handler(signum, frame):
    ...
    # For immediate response, also try to interrupt any running operations
    import threading
    main_thread = threading.main_thread()
    if main_thread != threading.current_thread():
        # We're in a signal handler, force wake up the main thread
        try:
            # This will help break out of blocking operations
            import _thread
...
def shutdown_server():
    ...
    def shutdown():
        ...
        try:
            # Explicitly remove lock file before terminating
            remove_lock_file()
        except Exception as e:
            logging.error(f"Error removing lock file during shutdown: {e}")
...
def create_tray_icon(port):
    ...
    try:
        ...
        def quit_app(icon, item):
            ...
            logging.info("Shutting down from system tray")
            remove_lock_file()
            icon.stop()

        def check_shutdown_flag(icon):
            """Check if shutdown was requested via signal"""
            global shutdown_requested
            if shutdown_requested:
                logging.info("Shutdown requested via signal, stopping tray icon")
                icon.stop()
        ...
        # Create the icon
        icon = pystray.Icon("The Bucket Wizard", image, menu=menu)
        icon.title = "The Bucket Wizard"
        
        # Set up periodic checking for shutdown signal
        import threading
            ...
            while True:
                if shutdown_requested:
                    try:
                        ...
                        pass
                    break
                time.sleep(0.05)  # Very frequent checking for immediate response
        
        # Store the monitor thread on the icon for cleanup
        icon._monitor_thread = threading.Thread(target=monitor_shutdown, daemon=True)
        
        logging.info("System tray icon object created successfully")
        return icon
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if os.path.exists(LOCK_FILE):
        show_popup(
            ...
            "The Bucket Wizard is already running.\n\n"
    ...
    logging.info("Starting The Bucket Wizard...")
    ...
    if args.web:
        ...
        if tray_icon:
            ...
            print("System tray icon created - right-click to access options")
            
            # Start the shutdown monitor thread
            tray_icon._monitor_thread.start()
            
            try:
                # Run the tray icon on main thread (this blocks until icon is stopped)
                # But with a setup function that makes it more responsive
                def setup_with_monitoring(icon):
                    logging.info("Tray icon is now visible")
                    # Make the icon visible
                    icon.visible = True
                
                tray_icon.run(setup=setup_with_monitoring)
                
            except KeyboardInterrupt:
                logging.info("KeyboardInterrupt received while running tray icon")
                shutdown_requested = True
                remove_lock_file()
                try:
                    tray_icon.stop()
                ...
                try:
                    # Fallback to simple console mode if tray fails
                    while True:
                        if shutdown_requested:
                            break
                        time.sleep(0.1)
                except KeyboardInterrupt:
                    logging.info("Keyboard interrupt received in tray fallback mode")
                    remove_lock_file()
        else:
